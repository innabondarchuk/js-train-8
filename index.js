...
function createArray(...elements) {
  ...
  if (elements.length > 0) {
    ...
    console.log("кількість переданих елементів менше нуля");
    return [];
  };
}
...
function findElementIndexes(arr, element) {
  ...
  if (!Array.isArray(arr)) {
    ...
    const indexFirst = arr.indexOf(element);
    const indexLast = arr.lastIndexOf(element);

    const indexesArr = [];

    if (indexFirst !== -1) {
      indexesArr.push(indexFirst);
    };

    if (indexLast !== indexFirst) {
      indexesArr.push(indexLast);
    };
...
function customIncludes(arr, element) {
  ...
  console.log(arr.filter(result => result === element).length);
  return result;
}
